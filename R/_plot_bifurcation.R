...
p_bifurcation <- ggplot() +
  ...
  geom_line(aes(x = rho, y = fixed_I),
            linewidth = 1.0,
            plot_data_bifurcation %>% filter(rho > 0.0065)) +
  geom_line(aes(x = rho, y = fixed_I),
...
p_examples <- cowplot::plot_grid(
  tibble(I = y_I_sol[which(x_rho == 0.005),]) %>% 
    mutate(t = row_number()) %>%
    ...
    geom_hline(yintercept = y_fixed_I[which(x_rho == 0.005)],
  ...
  tibble(I = y_I_sol[which(x_rho == 0.008),]) %>% 
    mutate(t = row_number()) %>%
    ...
    geom_hline(yintercept = maximums[which(x_rho == 0.008)],
               linewidth = 0.7, colour = colour_C) +
