...
ggplot() +
  ...
  #           linewidth = 0.7,
  #           plot_data_incidence %>% filter(eta == 0.1)) +
  ...
  ggtitle(NULL, "Average annual infection incidence")
...
ggplot() +
  ...
  geom_line(aes(x = r, y = inc_mean * 365),
            linewidth = 0.7,
            plot_data_incidence %>% filter(eta %in% c(0.1, 0.5))) +
  
  geom_line(aes(x = r, y = inc_mean * 365),
            colour = colour_C, linetype = "82",
            plot_data_incidence %>% filter(eta == 0) %>% ungroup() %>% select(-eta_label)) +
  
  facet_wrap(~eta_label, ncol = 2) +
...
ggplot() +
  ...
  ggtitle(NULL, "Proportional difference in<br>average annual infection incidence")
...
p_axes_freq <- ggplot() +
  ...
  geom_linerange(aes(ymin = 0, ymax = 0.02, x = x), expand_grid(x = c(0.0, freq_breaks_r)),
                 linewidth = 0.7) +
...
p_diff / (p_axes_freq | p_axes_freq)
