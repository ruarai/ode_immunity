...
model_params = make_model_parameters(
    ...
    b = b, h = h, c_jump_dist = c_jump_dist; boosting = "none", eta = 0.35
)

ode_sparsity = ode_get_sparsity(model_params)
ode_solution = ode_solve(model_params, n_days, n_inf_0, ode_sparsity, saveat = 0.25)
...
plot(y_zeroed[1:1:20000,:], legend = false)

sum_abs2 = vec(sum(abs2.(y_zeroed), dims = 2))


ϵ = 1e-6
plot(sum_abs2[1:end], ylims = (0.0, 0.00001))
hline!([ϵ])
...
period = mean(period_samples[2:end])
