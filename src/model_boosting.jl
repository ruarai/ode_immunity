function ode_step_boosting!(du, u, model_params, t)
    inf_incidence = ode_step_no_count_boosting!(du, u, model_params, t)
...
function ode_step_no_count_boosting!(du, u, model_params, t)
    ...
    @inbounds @simd for ix_sus in 1:S
        ...
        du[ix_inf] += strata_incidence - model_params.gamma_strata[ix_sus] * u[ix_inf]

        for i in 1:S
            ix_inf_inner = i + S;
            du[ix_sus] += u[ix_inf_inner] * model_params.M[ix_sus, i] * model_params.gamma_strata[i] 
        end
    end

    return inf_incidence
end